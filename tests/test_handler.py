...
async def test_handle_chat_completions_non_stream_aggregates_reasoning_content(backends_config, mock_logger):
    parser_matcher = ChunkConverterMatcher({"my-model": "reasoning", "default": "reasoning_content"}, mock_logger)
...
async def test_handle_chat_completions_native_stream_coalesces_reasoning_content_with_role(
    ...
    parser_matcher = ChunkConverterMatcher({"my-model": "reasoning"}, mock_logger)
    handler = ProxyHandler(
        backends_config,
        mock_logger,
        parser_matcher,
