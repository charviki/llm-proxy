...
class SSESemanticCoalescer:
    ...
    def _push_text(self, chunk: dict, text_state: _TextDelta, now_ms: int) -> list[dict]:
        ...
        if self._buffer is None:
            self._buffer = self._build_text_buffer(chunk, text_state, now_ms)
        else:
            merged_extra_delta = self._merge_extra_delta(self._buffer.extra_delta, text_state.extra_delta)
            # 文本字段类型变化（reasoning_content -> content）或上下文字段不兼容时，必须先 flush 再开启新 buffer。
            if self._buffer.field_name != text_state.field_name or merged_extra_delta is None:
                flushed.extend(self.flush_pending())
                self._buffer = self._build_text_buffer(chunk, text_state, now_ms)
            else:
                self._buffer.extra_delta = merged_extra_delta
                self._buffer.text += text_state.text
    ...
    def _push_tool_call(self, chunk: dict, tool_state: _ToolCallDelta, now_ms: int) -> list[dict]:
        if isinstance(self._buffer, _TextBuffer):
            flushed = self.flush_pending()
        elif isinstance(self._buffer, _ToolCallBuffer) and self._buffer.tool_index != tool_state.tool_index:
            flushed = self.flush_pending()
        else:
            flushed = []

        if self._buffer is None:
            self._buffer = self._build_tool_call_buffer(chunk, tool_state, now_ms)
        else:
            merged_extra_delta = self._merge_extra_delta(self._buffer.extra_delta, tool_state.extra_delta)
            if merged_extra_delta is None:
                ...
                return flushed
            self._buffer.extra_delta = merged_extra_delta
            self._buffer.arguments += tool_state.arguments
            if tool_state.tool_call_id:
                self._buffer.tool_call_id = tool_state.tool_call_id
            if tool_state.tool_call_type:
                self._buffer.tool_call_type = tool_state.tool_call_type
            if tool_state.function_name:
                self._buffer.function_name = tool_state.function_name
    ...
    def _build_text_buffer(self, chunk: dict, text_state: _TextDelta, now_ms: int) -> _TextBuffer:
        return _TextBuffer(
            envelope=self._capture_envelope(chunk),
            field_name=text_state.field_name,
            text=text_state.text,
            extra_delta=dict(text_state.extra_delta),
            started_at_ms=now_ms,
        )

    def _build_tool_call_buffer(self, chunk: dict, tool_state: _ToolCallDelta, now_ms: int) -> _ToolCallBuffer:
        return _ToolCallBuffer(
            envelope=self._capture_envelope(chunk),
            tool_index=tool_state.tool_index,
            arguments=tool_state.arguments,
            extra_delta=dict(tool_state.extra_delta),
            started_at_ms=now_ms,
            tool_call_id=tool_state.tool_call_id,
            tool_call_type=tool_state.tool_call_type,
            function_name=tool_state.function_name,
        )
    ...
    @staticmethod
    def _extract_text_delta(delta: dict) -> Optional[_TextDelta]:
        ...
        return _TextDelta(field_name=field_name, text=text, extra_delta=extra_delta)

    @staticmethod
    def _extract_tool_call(delta: dict) -> Optional[_ToolCallDelta]:
