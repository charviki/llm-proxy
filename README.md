...
> **💡 特别提示 (Windows + WSL 用户)**：如果你需要在 Windows 和 WSL 两端都通过代理访问目标域名（例如 Windows 上的应用和 WSL 中的 IDE 远程开发都需要走代理），那么你**必须在 Windows 宿主机和 WSL 子系统中都安装并信任该证书**，并且除非你的 WSL 已经配置了与 Windows 宿主机共享 DNS/Hosts，否则**两边都需要修改 hosts 文件**。如果你只在其中一端使用代理，只需在该端安装证书和配置 hosts 即可。
